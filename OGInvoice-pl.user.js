...
(function(){
    ...
    return {date: rdate, time: rtime, index:lidx};
      ...
      this.record = function(txt){
          $(txt).find('.msg').each(function(id, m){
              var invoice_pattern = /dostawa surowców przez obcą flotę\s(\d\d\.\d\d\.\d\d\d\d)\s(\d\d\:\d\d\:\d\d)\sOd\:\sPerymetr\sObrony\sFlota\sod\s(.+)\s\([^\[]+\[(\d\:\d{1,3}:\d{1,2})\]\)\sdostarczyła\ssurowce\sna\s[^\[]+\[(\d\:\d{1,3}:\d{1,2})\]\s\:Metal:([\d.]+)\sKryształ:([\d.]+)\sDeuter:([\d.]+)/;
              var clrStr = $(m).text().replace(/(\s)+/g,'$1');
              var abstract = invoice_pattern.exec(clrStr);
              if(abstract){
                  $(m).find('a.fright').remove()
                  var invoice = new Invoice($(m).data('msgId'), abstract[1], abstract[2], $($(m).find('.player').attr('title').replace(/Gracz\:[^\|]*\|/, '')).find('.sendMail.tooltip').data('playerid'), abstract[3], abstract[4], abstract[5], abstract[6], abstract[7], abstract[8], $(m).html());
                  oginv.push(invoice);
              }
          });
  ...
  this.refreshPanel = function(){
      ...
      this.refreshSettingPanel = function(){
          ...
          $("#oginv_contract_list").append("<table><tr><th>#</th><th>Player</th><th>Deuts per week</th><th>Trade on every</th><th>Delete</th></tr></table>");
          var count = 0;
