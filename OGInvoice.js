...
(function(){
    ...
    function OGInv() {
		//DATA 
        this.calQueue = [];
        this.info = {name: "OGInvoice", version: '3.0.0', author: "MomoCow", site: "https://github.com/momocow", description: "OGame: 自動追蹤/統計 交易資源量", statistic:[], weekly:[], storage: [], setting:{}};
            ...
            $.ajax({url:$(location).prop("protocol") + "//" + $(location).prop("host") +"/game/index.php?page=search",data:{searchValue:$('#oginv_input_search_player').val(),method:m,currentSite:p,ajax:"1"},
                        success:function(data){
                            ...
                            $(".oginv_to_contract").on("click", function(e){
                                ...
                                $("#oginv_input_contract_day").css({"width":"200px"}).on("change", function(e){if($("#oginv_input_contract_day").val() == -1) $("#oginv_input_contract_amount").prop("disabled", true);});
